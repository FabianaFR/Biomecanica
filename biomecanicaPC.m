...
Archivo='0029_Davis_Marcha01_Walking10b2021.c3d';
...
% %PIE IZQUIERDO
% CAMBIO
% Hay un problema en el cálculo de u, v y w de pie izquierdo ... no alcanzo
% a identificar cuál es, asique lo cambié por otra rutina (que a mí sí me
% funciona porque funciona el ./. Y ahí los ángulos dan parecidos a los del
% campus. Asique tendríamos que ver cómo solucionar esto: yo sugiero dos
% posibilidades: a) volver a escribir de manera muy cuidadosa tu rutina
% para el cálculo de u, v y w. O b) usar mi rutina y usar la funcion
% rdivide (mirá abajo como la implementaría).
...
Lupie= (p8-p9)./ sqrt(sum((p8-p9).^2,2));

%Podés probar la función rdivide ...fijate si te funciona en tu matlab
%(primero probala en el command window)

% Esto primero sería 
% Lupie= rdivide ((p8-p9), sqrt(sum((p8-p9).^2,2)));

% Cálculo de w right foot
Awi=(p8-p10);
Bwi=(p9-p10);
Cwi=cross(Awi,Bwi);
normucwi=sqrt(sum(Cwi.^2,2));
Lwpie=Cwi./normucwi;
